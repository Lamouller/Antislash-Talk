...
async function main() {
  ...
  try {
    // Séparer les captures selon l'authentification
    const publicScreenshots = SCREENSHOTS.filter(s => !s.requiresAuth);
    const authScreenshots = SCREENSHOTS.filter(s => s.requiresAuth);
    
    // 1. D'abord prendre les captures publiques (sans authentification)
    console.log('📸 Captures publiques (sans connexion)...\n');
    for (const screenshot of publicScreenshots) {
      await takeScreenshot(browser, screenshot);
      await new Promise(resolve => setTimeout(resolve, 1000));
    }
    
    // 2. Ensuite se connecter si nécessaire
    if (authScreenshots.length > 0) {
      console.log('\n🔐 Connexion nécessaire pour les captures suivantes...');
      ...
      // 3. Prendre les captures qui nécessitent l'authentification
